...
resource "google_service_account_iam_member" "workload_identity_link" {
  service_account_id = google_service_account.vertex_agent.name
  role               = "roles/iam.workloadIdentityUser"
  member             = "serviceAccount:seir-1-490120.svc.id.goog[default/vertex-agent-ksa]"
}
